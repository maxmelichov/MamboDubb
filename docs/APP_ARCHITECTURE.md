...
|---|---|---|---|
| `seed` | int 0..2³²-1 | derived | replaces the text-derived seed, on every attempt: the retry ladder varies the clone reference, not the seed. Bump it to re-roll a sampled take. A substitute reference is only offered when ECAPA says it is this segment's own voice, so a segment whose speaker label covers two people retries on its own audio instead of borrowing the other person's. |
